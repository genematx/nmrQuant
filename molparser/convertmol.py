...
def parse_counts_line(line):
    """
    Parse the counts/header line of a molecule and return it as a dictionary
    ...
    except ValueError: pass     # If the line is too short
    ret["vvvvv"] = line[-5:]
    return ret
...
def parse_atom_line(line):
    ...
    except ValueError: pass # If the string is too short
    return ret
...
def apply_m_rad(line,mol):
    ...
    for i in range(0,len(line),2):
        aaa = int(line[i])
        vvv = int(line[i+1])
        mol["?atom_%04d"%aaa]["radical"] = radical_dict[vvv]
    return mol
...
def apply_m_iso(line,mol):
    ...
    for i in range(0,len(line),2):
        aaa = int(line[i])
        vvv = int(line[i]+1)
        mol["?atom_%04d"%aaa]["mass_diff"] = "+"+radical_dict[vvv]
    return mol
...
def parse_mol(lines):
    ...
    if mol['version'] == 'V2000':
        num_atoms = c_line["aaa"]
        num_bonds = c_line["bbb"]
        num_lists = c_line["lll"]
        mol["chiral"] = (c_line["ccc"] == 1)

        # Begging and end lines of the atom and bond blocks
        atom_beg, atom_end = 4, 4 + num_atoms
        bond_beg, bond_end = atom_end, atom_end + num_bonds

    elif mol['version'] == 'V3000':
        c_line = lines[5].split()[3:]
        num_atoms = int(c_line[0])
        num_bonds = int(c_line[1])
        mol["chiral"] = (c_line[4]=='1')

        # Begging and end lines of the atom and bond blocks
        atom_beg, atom_end = 7, 7 + num_atoms
        bond_beg, bond_end = atom_end+2, atom_end+2 + num_bonds
    else: raise RuntimeEror('Unknown version: ', mol['version'])

    ### ATOM BLOCK ###
    for i, line in enumerate(lines[atom_beg:atom_end]):
        atom = {}
        if mol['version'] == 'V2000':
            atom['indx'] = i+1
            ...
            atom["hydrogen_count"] = a_line["hhh"]

        elif mol['version'] == 'V3000':
            a_line = line.split()[2:]
            atom['indx'] = int(a_line[0])   # Unique atom ID
            atom['symbol'] = a_line[1]
            atom["x"] = float(a_line[2])
            atom["y"] = float(a_line[3])
            atom["z"] = float(a_line[4])
            if '=' in a_line[-1]:
                comment = a_line[-1].split('=')
                atom[comment[0]] = comment[1]

        mol['atom_{:d}'.format(atom['indx'])] = atom

    ### BOND BLOCK ###
    for i, line in enumerate(lines[bond_beg:bond_end]):
        bond = {}
        if mol['version'] == 'V2000':
            bond['indx'] = i+1
            ...
            if bond['type'] == "Single":
                bond['stereo'] = single_bond_stereo_dict[b_line["sss"]]
            elif bond['type'] == "Double":
                bond['stereo'] = double_bond_stereo_dict[b_line["sss"]]
            bond['atom_ids'] = [b_line["111"], b_line["222"]]

        elif mol['version'] == 'V3000':
            b_line = line.split()[2:]
            bond['indx'] = int(b_line[0])
            bond['type'] = bond_type_dict[int(b_line[1])]
            bond['atom_ids'] = [int(b_line[2]), int(b_line[3])]    # Pair of atoms connected by the bond

        mol['bond_{:d}'.format(bond['indx'])] = bond

    ### PROPERTIES BLOCK ###

    Ms = []
    for line in lines[bond_end:]:
        if line == 'M  END' or len(line) == 0:
            break
        if line[0] == "M":
            Ms.append(line)
        else:
    ...
    ### DATA ITEMS ###
    data_beg = [i for i, line in enumerate(lines) if line.startswith('>')] + [len(lines)]   # Indices of first lines in each data block
    for beg, end in zip(data_beg[:-1], data_beg[1:]):
        data_lines = lines[beg:end]
        data_header = parse_data_name(data_lines[0])
        data_lines = [line for line in data_lines[1:] if len(line)>0]
        mol[data_header] = '\n'.join(data_lines)
...
if __name__ == '__main__':
    ...
    mols = parse_sdf_file("./testSDFs/Compound_000000001_000025000.sdf", True)
    pprint(mols[0])

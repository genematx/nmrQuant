...
from .baseline import _ps_acme_score, _ps_peak_minima_score, baseline_corrector, med
...
def reportMCMC(samples):
    ...
    tab.append(['Total', "{:.4f}".format(m_tot.item()), "{:.2f}".format(np.sqrt(v_tot).item()), ""])
...
def flatten(h, new_key=[]):
    ...
    items = []
    for k, v in h.items():
        if isinstance(v, dict):    # v is another dictionary
...
def deg2tau(dt, nf, p0deg, p1deg):
    ...
    theta = ( p0deg - p1deg*fmin/df/nf ).item()*np.pi/180.
    tau = ( p1deg / df / 360. / nf ).item()
